...
for arg in $args; do
    sum_s0=0
    for i in {1..10}; do
        sum_s0=$(( $(./aor2p res/bb.png $arg -o=benchmark_output.png -s0 | grep 'time taken' | awk '{print $3}' | rev | cut -c 3- | rev) + $sum_s0 ))
    done 

    sum_s3=0
    for i in {1..10}; do
        sum_s3=$(( $(./aor2p res/bb.png $arg -o=benchmark_output.png $@ | grep 'time taken' | awk '{print $3}' | rev | cut -c 3- | rev) + $sum_s3 ))
    done
