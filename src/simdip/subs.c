...
void simd_subs_bmp_8bpc(uint8_t* ptr_r, uint8_t* ptr_g, uint8_t* ptr_b, uint8_t c){
...
void simd_subs_bmp_8bpc_npl(imgfile_t* imgfile, uint8_t c){
    for(size_t i = 0; i < imgfile->height; ++i){
        ...
        for(; j < (imgfile->width & ~0x1F); j += 32){
            simd_subs_bmp_8bpc(ptr_r + j, ptr_g + j, ptr_b + j, c);
        }

        for(; j < imgfile->width; ++j){
            *ptr_r++ = *ptr_r - c < 0 ? 0 : *ptr_r - c;
            *ptr_g++ = *ptr_g - c < 0 ? 0 : *ptr_g - c;
            *ptr_b++ = *ptr_b - c < 0 ? 0 : *ptr_b - c;
        }
    }
}

// no simd, subtract constant from pixel, .bmp, 8 bits per channel, no pipeline
void subs_bmp_8bpc_npl(imgfile_t* imgfile, uint8_t c){
    for(size_t i = 0; i < imgfile->height; ++i){
