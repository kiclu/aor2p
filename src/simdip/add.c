...
void simd_add_bmp_8bpc(__m256i* ptr_r, __m256i* ptr_g, __m256i* ptr_b, uint8_t c){
    ...
    _mm256_store_si256(ptr_r, _mm256_add_epi8(va_r, vc));
    _mm256_store_si256(ptr_g, _mm256_add_epi8(va_g, vc));
    _mm256_store_si256(ptr_b, _mm256_add_epi8(va_b, vc));
}

#include<stdio.h>

// simd, add constant, .bmp, 8 bits per channel, no pipeline
void simd_add_bmp_8bpc_npl(imgfile_t* imgfile, uint8_t c){
    printf("simd_add_bmp_8bpc_npp\n");
...
void add_bmp_8bpc_npl(imgfile_t* imgfile, uint8_t c){
    printf("add_bmp_8bpc_npp\n");
